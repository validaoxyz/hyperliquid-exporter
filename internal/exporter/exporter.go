...
func Start(ctx context.Context, cfg config.Config) {
	...
	accumulatorConsensusErrCh := make(chan error, 1)
	replayErrCh := make(chan error, 1)
	mempoolErrCh := make(chan error, 1)
	mempoolTxsErrCh := make(chan error, 1)
	...
	// peer-set monitor (rolling windowed unique-peer counts; optional
	// per-IP gauges under --per-peer-metrics). Fed by tcp_traffic.
	...
	runMonitor("accumulator_consensus", func() { monitors.StartAccumulatorConsensusMonitor(monitorCtx, cfg, accumulatorConsensusErrCh) })

	// replay-event count from data/node_logs/replay/<height>_<iso>/ subdirs
	logger.InfoComponent("replay", "Initializing replay-event monitor...")
	runMonitor("replay", func() { monitors.StartReplayMonitor(monitorCtx, cfg, replayErrCh) })

	// mempool log streamer (validator-only, heavy log; offset-tracked)
	logger.InfoComponent("mempool", "Initializing mempool monitor...")
	runMonitor("mempool", func() { monitors.StartMempoolMonitor(monitorCtx, cfg, mempoolErrCh) })

	// split-client mempool transaction stream. Present on nodes with
	// split_client_blocks enabled; idles elsewhere.
	logger.InfoComponent("mempool_txs", "Initializing mempool_txs monitor...")
	runMonitor("mempool_txs", func() { monitors.StartMempoolTxsMonitor(monitorCtx, cfg, mempoolTxsErrCh) })
	...
	for {
		...
		case err := <-peerSetErrCh:
			metrics.IncMonitorError("peer_set")
			logger.ErrorComponent("peer_set", "Peer-set monitor error: %v", err)
		...
		case err := <-accumulatorConsensusErrCh:
			metrics.IncMonitorError("accumulator_consensus")
			logger.ErrorComponent("accumulator_consensus", "accumulator-consensus monitor error: %v", err)
		case err := <-replayErrCh:
			metrics.IncMonitorError("replay")
			logger.ErrorComponent("replay", "replay monitor error: %v", err)
		case err := <-mempoolErrCh:
			metrics.IncMonitorError("mempool")
			logger.ErrorComponent("mempool", "mempool monitor error: %v", err)
		case err := <-mempoolTxsErrCh:
			metrics.IncMonitorError("mempool_txs")
