...
- **Cardinality**: any per-IP / per-peer / per-validator / per-anything label needs an explicit ceiling. We use three patterns:
  ...
  - For raw order-flow sources such as `mempool_txs`, never label by hash, address, asset, price, order id, cloid, or nonce. Export aggregate counts/histograms only.
...
- **Stream files seek to EOF on startup**. Restart means data loss for in-flight records. Documented limitation. On hourly rotation, start the new file at offset 0 so the first records of the hour are not dropped.
